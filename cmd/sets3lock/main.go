...
func _main() int {
	...
	flag.BoolVar(&x, "x", false, "If lock object creation/update fails or the lock cannot be obtained, sets3lock exits zero.")
	flag.BoolVar(&X, "X", false, "(Default.) If lock object creation/update fails or the lock cannot be obtained, sets3lock prints an error message and exits nonzero.")
